...
class VBA_class(nn.Module):
    ...
    def __init__(self, test_conditions, folders, mode='first_layer', 
                 lr_first_layer=5e-3, lr_greedy=1e-3, lr_lpp=1e-3, lr_N_N = 5e-5, 
                 nb_epochs=[10,10,200,6], nb_blocks=22, batch_size=[10,10,10], device = 'GPU'):
        ...
        if mode == 'test':
            self.model        = myModel_RGB(self.dtype,0,0,self.nb_blocks).type(self.dtype)
        else:
            KmtK0_dict, KmtKn_dict = init_settings0(mode)
            self.model        = myModel_RGB(self.dtype,KmtK0_dict,KmtKn_dict,self.nb_blocks).type(self.dtype)
        
        # the post-processing layer
        self.last_layer   = myLastLayer_RGB().type(self.dtype)
...
#########################################################################################
    ...
    def save_OneBlock(self,block=0): 
        ...
        for minibatch in self.train_loader:
            if self.mode=='first_layer':
                [names, x_true_RGB, x_true, x_blurred_RGB, x_blurred, h] = minibatch 
                if names =='.ipynb_checkpoints': continue
                print('The name is ',names)     
                x_true       = Variable(x_true.type(self.dtype),requires_grad=False)
                x_blurred    = Variable(x_blurred.type(self.dtype),requires_grad=False)
    ...
    def test(self, dataset):  
        ...
        """       
        path_savetest_mk                   = os.path.join(self.path_save,'Results_on_Testsets','Flickr30','mk_vec_greedy')
        path_savetest_newmh                = os.path.join(self.path_save,'Results_on_Testsets','Flickr30','newmh_vec_greedy')
        ...
        data          = MyTestset_RGB(folder=path_dataset)
        ...
        for minibatch in tqdm(loader,file=sys.stdout):
            ...
            mk_vec_RGB[j,:,:].detach().cpu().numpy().astype('float64')})
            ...
            loss_mk                = self.loss_fun_mk(mk_vec_RGB, x_true_)
