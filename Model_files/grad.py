...
def compute_grad_h_x(beta,sizeh,mkvector_vec,yvector_vec,newmh_vec,newSigmah_vec,newk,r_vec,rt_vec,symmetry,dtype):
    ...
    grad_g4_f2_vec = -2*beta*torch.transpose(KntKmf2_vec,1,2)+beta*newSigmah_vec@torch.transpose(Kmty_vec-2*K0tKmf2_vec,1,2)
    newSigmah_vec_gpu = newSigmah_vec.reshape(batch,1,newMh,newMh).type(dtype)
    grad_g3_f2_vec = -2*beta*torch.transpose(newSigmah_vec_gpu@torch.transpose(CmnKntKmf2_vec,1,2),1,2)
...
def compute_grad_C_h_vec(beta,newMh,mkvector_vec,h_vec,grad_C_A_vec,KmtK0_dict,KmtKn_dict,dtype):
    ...
    grad_C_h_vec = -beta*(2*torch.sum(h_vec*KmtKn_vec,1)*h_vec.reshape(batch,newMh)+KmtK0_vec)
    grad_C_Ch_vec = -beta*KmtKn_vec
