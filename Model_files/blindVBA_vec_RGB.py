...
from Model_files.compute_grad_RGB import *


class cardan_RGB(torch.autograd.Function):  

    @staticmethod
    def forward(ctx,y_vec,mk0_vec,h0_vec,beta,xi,h_vec,Ch_vec,gamma_vec,lambda_vec,KmtK0_dict,KmtKn_dict,dtype,mode_training=True):
        print('gvar is ',(1/beta)**0.5)
        ...
        newbx_vec= Xt_vec(y_vec)*beta.unsqueeze(-1).unsqueeze(-1)
        
        # Update X

        print ('--- update x')
        
        (mk_vec,diagnewterm_vec)= linear_solver_gaussian_vec_RGB(beta,sizex,sizeh,X_vec,Xt_vec,B_vec,Bt_vec,P_vec,newbx_vec,Ncg,mk0_vec,tolcg,newSigmah_vec,newk,diag_qudratick,r_vec,rt_vec,symmetry,dtype)
     
        (DdiagSigma_vec,diagSigma_vec)= computeDiagSigma_vec_RGB(beta,diagXtX_vec,diagnewterm_vec,P_vec,mk_vec,B_vec,Babs_vec,Btabs_vec)
        ...
        A3_vec = avector_vec*beta.unsqueeze(-1) 
        L_vec = L.repeat(batch,1,1)
        A4_vec = L_vec@newmuh_vec.type(dtype).unsqueeze(-1) 
        bhvector_vec= A3_vec.unsqueeze(-1) + A4_vec*Xip.unsqueeze(-1).unsqueeze(-1)
        A1_vec = Bmatrix_vec*beta.unsqueeze(-1).unsqueeze(-1)
        A2_vec = L_vec 
        ...
        if mode_training==True:
            print('It is saved')
            ctx.save_for_backward(Xip,beta,avector_vec,Bmatrix_vec,A1_vec,A2_vec,A3_vec,A4_vec,bGammapos_vec,h0_vec,h_vec,newmh_vec,Ch_vec,newSigmah_vec,diagSigma_vec,mk0_vec,mkvector_vec,yvector_vec,lambda_vec,LAMBDAk1_vec,gamma_vec,Gammap_vec,P_vec)
    ...
    def backward(ctx, grad_outputx, grad_outputCx, grad_outputh, grad_outputCh, grad_outputgamma, grad_outputlambda):#grad for outputs
        ...
        Xip,beta,avector_vec,Bmatrix_vec,A1_vec,A2_vec,A3_vec,A4_vec,bGammapos_vec,h0_vec,h_vec,newmh_vec,Ch_vec,newSigmah_vec,diagSigma_vec,mk0_vec,mkvector_vec,yvector_vec,lambda_vec,LAMBDAk1_vec,gamma_vec,Gammap_vec,P_vec    = ctx.saved_tensors
        Xip = Xip
...
#             KmtKn_dict = pickle.loads(handle.read())
        ...
        grad_h_beta        = invsigmah_vec@(-Bmatrix_vec@invsigmah_vec@bhvector_vec+avector_vec.unsqueeze(-1))
        grad_Ch_beta        = -invsigmah_vec@Bmatrix_vec@invsigmah_vec 
        ...
        # grad of outputs wrt beta
        grad_C_beta_vec        = torch.sum(grad_outputh*grad_h_beta,dim=(1,2)) + torch.sum(grad_outputCh*grad_Ch_beta,dim=(1,2))#1
        print('BP for beta finished')
        ...
        grad_C_h_vec,grad_C_Ch_vec,grad_C_lambda_vec,grad_C_gamma_vec = compute_grad_RGB(dtype,settings, constant, grad, variables)
        ...
        grad_input_beta       = Variable(grad_C_beta_vec.type(dtype),requires_grad=True)

        print('BP finished')

        return None,None,None,grad_input_beta,grad_input_xi, grad_input_h, grad_input_Ch, grad_input_gamma, grad_input_lambda,None,None,None,None
