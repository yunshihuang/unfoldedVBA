...
from Model_files.grad_RGB import *
from Model_files.modules import *
from torch.nn.modules.loss import _Loss
import gc
import scipy

def compute_grad_RGB(dtype,settings, constant, grad, variables):
    # settings
    sizeh = settings[0]
    sizex = settings[1]
    newMh = settings[2]
    ...
    if (torch.norm(grad_outputCh)+torch.norm(grad_outputlambda)+torch.norm(grad_outputgamma)).cpu().numpy() == 0:
        if torch.norm(grad_outputx).cpu().numpy() == 0:#last layer of the greedy approach
            ...
            grad_g4_f0_vec = -A1_vec*beta.unsqueeze(-1).unsqueeze(-1)+A2_vec*beta.unsqueeze(-1).unsqueeze(-1)


            Kmkn_vec = 2*torch.sum(h_vec*diag_qudratick_gpu[:newMh,:newMh],dim=1).unsqueeze(-1)*h_vec

            grad_f0_h_vec = -(Kmkn_vec+Isum2_vec)*beta.unsqueeze(-1).unsqueeze(-1)
            ...
            # grad_h_Ch
            grad_f0_Ch_vec = -diag_qudratick_gpu[:newMh,:newMh]*beta.unsqueeze(-1).unsqueeze(-1)
            grad_g4_Ch_vec = (grad_g4_f0_vec@grad_f0_Ch_vec.reshape(batch,1,newMh**2))*torch.sum(diagSigma_vec**2,dim=(1,2)).reshape(batch,1,1)
            ...
            grad_C_A_vec,none= linear_solver_gaussian_vec_RGB(beta,sizex,sizeh,X_vec,Xt_vec,B_vec,Bt_vec,P_vec,grad_C_x_vec,Ncg,mk0_vec,tolcg,Ch_vec,newk,diag_qudratick,r_vec,rt_vec,symmetry,dtype) 
        ...
        # grad_Ch_h
        grad_g3_f0_vec = -(newSigmah_vec@diag_qudratick_gpu[:newMh,:newMh]@newSigmah_vec)*beta.unsqueeze(-1).unsqueeze(-1)
        ...
        grad_g3_Ch_vec_p1 = (grad_g3_f0_vec.reshape(batch,newMh**2,1)@grad_f0_Ch_vec.reshape(batch,1,newMh**2))*torch.sum(diagSigma_vec**2,dim=(1,2)).reshape(batch,1,1)
        ...
        # grad_lambda_Ch
        grad_C_Ch_vec_p1_lambda =  torch.sum(grad_outputlambda*grad_g5_f0_vec)*grad_f0_Ch_vec 

        # grad_gamma_Ch
        grad_C_Ch_vec_p1_gamma =  torch.sum(grad_outputgamma.reshape(batch,1,1)*grad_f6_f0_vec)*grad_f0_Ch_vec 
        ...
        # grad_h_lambda
        grad_f0_lambda_vec = -2*kappa*(kappa-1)*gamma_vec*torch.pow(lambda_vec,kappa-2)#batch*sizex*sizex
        grad_C_lambda_vec_p1_h = torch.sum(grad_outputh*grad_g4_f0_vec)*grad_g5_f0_vec*grad_f0_lambda_vec 
